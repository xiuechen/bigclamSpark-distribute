 ...
 * before the Dataset has been materialized.  After updating [[MyPeriodicCheckpointer]], users are
 ...
 */
abstract class MyPeriodicCheckpointer[T](
  ...
  private def removeCheckpointFile(): Unit = {
    ...
    getCheckpointFiles(old).foreach(
      MyPeriodicCheckpointer.removeCheckpointFile(_, sc.hadoopConfiguration))
  }
}

object MyPeriodicCheckpointer extends Logging {
  ...
  def removeCheckpointFile(checkpointFile: String, conf: Configuration): Unit = {
    try {
      ...
      case e: Exception =>
        logWarning("MyPeriodicCheckpointer could not remove old checkpoint file: " +
          checkpointFile)
    }
  }
}
