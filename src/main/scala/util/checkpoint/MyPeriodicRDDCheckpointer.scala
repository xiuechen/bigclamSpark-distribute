 ...
 * before the RDD has been materialized.  After updating [[MyPeriodicRDDCheckpointer]], users are
 ...
 * {{{
 *  val (rdd1, rdd2, rdd3, ...) = ...
 *  val cp = new MyPeriodicRDDCheckpointer(2, sc)
 *  cp.update(rdd1)
 *  rdd1.count();
 *  // persisted: rdd1
 *  cp.update(rdd2)
...
class MyPeriodicRDDCheckpointer[T](
    checkpointInterval: Int,
    sc: SparkContext,
    storageLevel: StorageLevel)
  extends MyPeriodicCheckpointer[RDD[T]](checkpointInterval, sc) {
